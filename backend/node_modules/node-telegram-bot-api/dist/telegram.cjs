...
const polling_js_1 = require("./polling.cjs");
const webhook_js_1 = require("./webhook.cjs");
const utils_js_1 = require("./utils.cjs");
const schemas_js_1 = require("./types/schemas.cjs");
const errors = __importStar(require("./errors.cjs"));
const debug = (0, debug_js_1.default)("node-telegram-bot-api");
...
class TelegramBot extends node_events_1.EventEmitter {
    ...
    constructor(token, options = {}) {
        ...
        this.http = new http_js_1.HttpClient(token, {
        ...
        if (this.options.polling) {
            ...
            this._polling = new polling_js_1.TelegramBotPolling(this, pollingOpts);
            if (autoStart)
            ...
            this._webHook = new webhook_js_1.TelegramBotWebHook(this, webhookOpts);
            if (autoOpen)
    ...
    _fixReplyMarkup(obj) {
        ...
        if (replyMarkup && typeof replyMarkup !== "string") {
            obj.reply_markup = (0, utils_js_1.stringify)(replyMarkup);
        }
    }
    ...
    _fixSuggestedPostParameters(obj) {
        if (Object.prototype.hasOwnProperty.call(obj, "suggested_post_parameters") &&
            ...
            obj.suggested_post_parameters = (0, utils_js_1.stringify)(obj.suggested_post_parameters);
        }
    }
    _fixLinkPreviewOptions(obj) {
        const value = obj.link_preview_options;
        if (value && typeof value !== "string") {
            obj.link_preview_options = (0, utils_js_1.stringify)(value);
        }
    }
    _fixStoryAreas(obj) {
        const value = obj.areas;
        if (value && typeof value !== "string") {
            obj.areas = (0, utils_js_1.stringify)(value);
        }
    }
    _fixMessageIds(obj) {
        const messageIds = obj.message_ids;
        if (messageIds && typeof messageIds !== "string") {
            obj.message_ids = (0, utils_js_1.stringify)(messageIds);
        }
    }
    /**
    ...
    async _sendFile(method, fieldName, data, form, fileMeta = {}, extraThumbnail) {
        const opts = { form };
        const { file, fileId } = await (0, utils_js_1.prepareFile)(data, fileMeta, this.options.filepath);
        if (file) {
        ...
        if (extraThumbnail) {
            ...
            if (candidate) {
                const { file: thumbFile, fileId: thumbId } = await (0, utils_js_1.prepareFile)(candidate, {}, this.options.filepath);
                if (thumbFile) {
    ...
    async _buildMediaItems(media) {
        ...
        for (const [index, item] of media.entries()) {
            ...
            for (const field of ["media", "thumbnail", "cover", "photo"]) {
                ...
                const { file, fileId } = await (0, utils_js_1.prepareFile)(value, meta, this.options.filepath);
                if (file) {
    ...
    async _buildStickerItems(stickers) {
        ...
        for (const [index, item] of stickers.entries()) {
            ...
            const { file, fileId } = await (0, utils_js_1.prepareFile)(data, fileOptions, this.options.filepath);
            if (file) {
                formData[attachName] = file;
    ...
    async startPolling(options = {}) {
        if (this.hasOpenWebHook()) {
            throw new errors_js_1.FatalError("Polling and WebHook are mutually exclusive");
        }
        if (!this._polling) {
    ...
    async openWebHook() {
        if (this.isPolling())
            throw new errors_js_1.FatalError("WebHook and Polling are mutually exclusive");
        if (!this._webHook) {
    ...
    processUpdate(update) {
        ...
        for (const key of schemas_js_1.UPDATE_TYPES) {
        ...
        void (async () => {
            try {
                ...
                if (!response.ok || !response.body) {
                    throw new errors_js_1.FatalError(`Failed to fetch file: HTTP ${response.status}`);
                }
                await (0, promises_1.pipeline)(node_stream_1.Readable.fromWeb(response.body), out);
            }
            catch (err) {
                out.emit("error", err);
            }
    ...
    async downloadFile(fileId, downloadDir, options = {}) {
        ...
        const filePath = node_path_1.default.join(downloadDir, fileName);
        const response = await fetch(uri);
        if (!response.ok || !response.body) {
            throw new errors_js_1.FatalError(`Failed to download file: HTTP ${response.status}`);
        }
        const out = (0, node_fs_1.createWriteStream)(filePath);
        await (0, promises_1.pipeline)(node_stream_1.Readable.fromWeb(response.body), out);
        return filePath;
    }
    // --- Updates / webhook -------------------------------------------------
    getUpdates(form = {}) {
        if (Array.isArray(form.allowed_updates)) {
            form.allowed_updates = (0, utils_js_1.stringify)(form.allowed_updates);
        }
    ...
    async setWebhook(url, options = {}, fileOptions = {}) {
        ...
        if (Array.isArray(form.allowed_updates))
            form.allowed_updates = (0, utils_js_1.stringify)(form.allowed_updates);
        if (certificate) {
            const { file, fileId } = await (0, utils_js_1.prepareFile)(certificate, fileOptions, this.options.filepath);
            if (file) {
    ...
    setWebHook(url, options = {}, fileOptions = {}) {
        (0, utils_js_1.deprecate)("setWebHook(...) is deprecated; use setWebhook(...) instead.");
    ...
    deleteWebHook(form = {}) {
        (0, utils_js_1.deprecate)("deleteWebHook(...) is deprecated; use deleteWebhook(...) instead.");
        return this.deleteWebhook(form);
    }
    /** @deprecated Renamed to {@link TelegramBot.getWebhookInfo} to match the Bot API casing. */
    getWebHookInfo(form = {}) {
        (0, utils_js_1.deprecate)("getWebHookInfo(...) is deprecated; use getWebhookInfo(...) instead.");
    ...
    copyMessages(chatId, fromChatId, messageIds, form = {}) {
        return this._form("copyMessages", {
            ...
            chat_id: chatId,
            from_chat_id: fromChatId,
            message_ids: (0, utils_js_1.stringify)(messageIds),
        });
    }
    // --- Send file methods -------------------------------------------------
    ...
    async sendLivePhoto(chatId, livePhoto, photo, options = {}, fileOptions = {}) {
        ...
        const opts = { form };
        const liveResult = await (0, utils_js_1.prepareFile)(livePhoto, fileOptions, this.options.filepath);
        if (liveResult.file) {
        ...
        const photoResult = await (0, utils_js_1.prepareFile)(photo, {}, this.options.filepath);
        if (photoResult.file) {
            opts.formData = opts.formData ?? {};
    ...
    async sendPaidMedia(chatId, starCount, media, options = {}) {
        ...
        form.media = (0, utils_js_1.stringify)(inputMedia);
    ...
    sendPoll(chatId, question, pollOptions, form = {}) {
        const out = { ...form, chat_id: chatId, question, options: (0, utils_js_1.stringify)(pollOptions) };
        if (out.country_codes)
            out.country_codes = (0, utils_js_1.stringify)(out.country_codes);
        if (out.correct_option_ids)
            out.correct_option_ids = (0, utils_js_1.stringify)(out.correct_option_ids);
    ...
    sendChecklist(businessConnectionId, chatId, checklist, form = {}) {
        return this._form("sendChecklist", {
            ...
            chat_id: chatId,
            checklist: (0, utils_js_1.stringify)(checklist),
        });
    }
    ...
    setMessageReaction(chatId, messageId, form = {}) {
        ...
        if (out.reaction)
            out.reaction = (0, utils_js_1.stringify)(out.reaction);
    ...
    restrictChatMember(chatId, userId, permissions, form = {}) {
        return this._form("restrictChatMember", {
            ...
            chat_id: chatId,
            user_id: userId,
            permissions: (0, utils_js_1.stringify)(permissions),
        });
    }
    ...
    answerGuestQuery(guestQueryId, result) {
        return this._form("answerGuestQuery", {
            ...
            result: (0, utils_js_1.stringify)(result),
        });
    }
    ...
    setManagedBotAccessSettings(userId, isAccessRestricted, form = {}) {
        ...
        if (form.added_user_ids)
            out.added_user_ids = (0, utils_js_1.stringify)(form.added_user_ids);
        return this._form("setManagedBotAccessSettings", out);
    }
    // --- Bot identity (self-management) ----------------------------------
    setMyCommands(commands, form = {}) {
        const out = { ...form, commands: (0, utils_js_1.stringify)(commands) };
        if (out.scope)
            out.scope = (0, utils_js_1.stringify)(out.scope);
    ...
    async setMyProfilePhoto(photo, form = {}) {
        ...
        const { file } = await (0, utils_js_1.prepareFile)(fileInput, {}, this.options.filepath);
        if (!file) {
            throw new errors_js_1.FatalError("setMyProfilePhoto only supports file uploads (Buffer, Stream, or local path)");
        }
        const struct = { ...photo, [fieldName]: `attach://${fieldName}` };
        return this._request("setMyProfilePhoto", {
            form: { ...form, photo: (0, utils_js_1.stringify)(struct) },
    ...
    async editMessageMedia(media, form = {}) {
        ...
        const out = {
            ...form,
            media: (0, utils_js_1.stringify)(inputMedia[0]),
        };
    ...
    async createNewStickerSet(params) {
        ...
        const form = { ...rest };
        form.stickers = (0, utils_js_1.stringify)(stickers);
    ...
    async addStickerToSet(params) {
        ...
        const form = { ...rest };
        form.sticker = (0, utils_js_1.stringify)(stickers[0]);
    ...
    sendInvoice(chatId, title, description, payload, providerToken, currency, prices, form = {}) {
        const out = {
            ...
            currency,
            prices: (0, utils_js_1.stringify)(prices),
        };
        if (out.provider_data !== undefined)
            out.provider_data = (0, utils_js_1.stringify)(out.provider_data);
        if (out.suggested_tip_amounts)
            out.suggested_tip_amounts = (0, utils_js_1.stringify)(out.suggested_tip_amounts);
    ...
    answerShippingQuery(shippingQueryId, ok, form = {}) {
        ...
        if (out.shipping_options)
            out.shipping_options = (0, utils_js_1.stringify)(out.shipping_options);
    ...
    async setBusinessAccountProfilePhoto(businessConnectionId, photo, form = {}) {
        ...
        const { file } = await (0, utils_js_1.prepareFile)(fileInput, {}, this.options.filepath);
        if (!file) {
            throw new errors_js_1.FatalError("setBusinessAccountProfilePhoto only supports file uploads (Buffer, Stream, or local path)");
        }
        const struct = { ...photo, [fieldName]: `attach://${fieldName}` };
        return this._request("setBusinessAccountProfilePhoto", {
            form: { ...form, photo: (0, utils_js_1.stringify)(struct), business_connection_id: businessConnectionId },
            formData: { [fieldName]: file },
        });
    }
    ...
    async postStory(businessConnectionId, content, activePeriod, options = {}) {
        if (!content.type)
            throw new errors_js_1.FatalError("content.type is required");
        const form = {
            ...options,
            business_connection_id: businessConnectionId,
        ...
        const { formData, fileIds } = await (0, utils_js_1.prepareFiles)(content.type, [{ media: fileInput }], {}, this.options.filepath);
        const inputContent = { ...content };
        inputContent[content.type] = fileIds[0] ?? `attach://${content.type}_0`;
        form.content = (0, utils_js_1.stringify)(inputContent);
