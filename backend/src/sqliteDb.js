...
function seedDatabase() {
  const insertProfile = sqlite.prepare(`
    INSERT INTO profiles (id, full_name, phone, role, is_active, avatar_badge, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  // 1. Profiles
  insertProfile.run('owner-1', 'Jasur Xidoyatov (Boshqaruvchi)', '+998 90 123 45 67', 'owner', 1, '👑', now);
  insertProfile.run('barber-1', 'Anvar Usta', '+998 93 111 22 33', 'barber', 1, '💈', now);
  insertProfile.run('barber-2', 'Bekzod Usta', '+998 94 444 55 66', 'barber', 1, '✂️', now);
  insertProfile.run('barber-3', 'Sardor Usta', '+998 97 777 88 99', 'barber', 1, '🧴', now);
